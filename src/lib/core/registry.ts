 ...
 * `currency` in Week 4, `markets` in Week 5, `rss` in Week 6. `core/registry.test.ts` checks each *registered* manifest against its
 * row in doc 06 §7, so the table stays authoritative without failing on widgets
 * that do not exist yet.
 */
export const MANIFESTS: readonly TpWidgetManifest[] = [
	clock,
	timer,
	calc,
	notes,
